...
local function debug_log(msg, config)
  if config and config.behavior and config.behavior.debug then
    vim.notify('[subjoyer] ' .. msg, vim.log.levels.INFO)
  end
end

-- Show error with deduplication (prevent spam)
local function show_error(msg, config, force)
...
local function handle_message(data, config)
  ...
  elseif data.type == 'disconnected' or data.type == 'client_disconnected' then
    ...
    debug_log('Client disconnected', config)
  ...
  elseif data.type == 'heartbeat' then
    ...
    debug_log('Heartbeat received', config)
  end
end
...
function M.start(config)
  ...
  local cmd = {
    ...
    '--port', tostring(config.connection.port),
  }
    ...
    on_stdout = function(_, data, _)
      for _, line in ipairs(data) do
        if line and line ~= '' then
          ...
          elseif config.behavior.debug then
            -- Only show parse errors in debug mode
            debug_log('Parse error: ' .. err, config)
          end
        end
      end
    end,
  ...
  if M.job_id <= 0 then
    ...
    show_error('Failed to start WebSocket server job', config, true)
...
function M.schedule_reconnect(config)
  ...
  if max_reconnects > 0 and M.reconnect_count >= max_reconnects then
    ...
    if not M.error_shown then
      debug_log(
        string.format('Max reconnection attempts (%d) reached. Use :SubjoyerReconnect to retry.', max_reconnects),
        config
      )
      M.error_shown = true
    end
  ...
  debug_log(
    string.format('Reconnecting in %dms (attempt %d/%d)', delay, M.reconnect_count, max_reconnects > 0 and max_reconnects or '∞'),
    config
  )
