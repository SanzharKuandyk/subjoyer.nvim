...
function M.setup(user_config)
    ...
    if not ok then
        vim.notify("[subjoyer] Failed to setup display provider", vim.log.levels.ERROR)
        return
    end
...
function M.setup_callbacks()
    ...
    websocket.on("ready", function(data)
        if cfg.behavior.debug then
            vim.notify(string.format("[subjoyer] Server ready on %s:%d", data.host, data.port), vim.log.levels.INFO)
        end
    end)
...
function M.setup_asbplayer_callbacks()
    ...
    asbplayer.on("ready", function(data)
        if cfg.asbplayer and cfg.asbplayer.debug then
            vim.notify(
                "[subjoyer:asbplayer] Server ready at " .. (data.url or "ws://127.0.0.1:8766/ws"),
                vim.log.levels.INFO
            )
        end
    end)
...
-- Show display bar
function M.show()
    if config.get().display.enabled then
        display.show()
    end
end
...
function M.toggle_display()
    local cfg = config.get()
    if not cfg.display.enabled then
        return
    end
    display.toggle(cfg)
end
...
function M.status()
    ...
    return {
        ...
        display_visible = display.is_visible == true,
...
function M.set_position(position)
    ...
    -- Need to recreate nui popup for position change
...
function M.mine_anki()
    ...
    if not status.is_connected then
        vim.notify(
            "[subjoyer] asbplayer not connected. Enable WebSocket client in asbplayer extension settings.",
